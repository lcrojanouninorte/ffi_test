    ...
    function instrument($log, $http, Restangular, backendCurrentUser) {
        ...
        function getAnswers(type) {
            ...
            if(user != null){
                ...
                return $http.get(apiHost + type + "?" + "user_id=" + user.id + "&app_id=" + user.app_id + "&auth_token=" + user.auth_token)
...
// .constant('user_nit', Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und[0].value) //logged drupal user
 ...
 //return $cookies.get('user_id');
            if (typeof(User) !="undefined") { //Verificar que el modulo envio la info
                //verificar si hay informacion del perfil2 ""beneficiarios""
                if (User != null) {
                    $log.debug("User profiel", User);
                    var nit = "";
                    var id = "";
                    var empresa = "";
                    ...
                    if (!angular.isUndefined(User.nit)) {
                        nit = User.nit ; 
                    }
                    if (!angular.isUndefined(User.empresa)) {
                        empresa = User.empresa ; 
                    } 
                    if (!angular.isUndefined(User.auth_token)) {
                        if(User.auth_token){
                            backend_auth_token = User.auth_token;
                            id = User.id_api;  
                            app_id = User.app_id;
                        }
                    } 
...
(function() {
    ...
    InstrumentsController.$inject = ["$cookies", "$window", "$log", "drupal", "$scope"];
    angular
        .module('InnovationManagement')
        .controller('InstrumentsController', InstrumentsController);

     /** @ngInject */
    function InstrumentsController($cookies, $window,$log, drupal, $scope) {
      var vm = this;
       $scope.variable1 = $window.User;
       $log.debug("user en instrumen",$scope.variable1);
       $log.debug("user as gloabl",User);
