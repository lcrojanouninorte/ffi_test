...
(function() {
  ...
  angular
    .module('InnovationManagement', [ 'ngAnimate','toaster',  'ngTouch', 'ngSanitize', 'restangular', 'ui.router', 'ui.bootstrap','vButton' , 'vr.directives.slider', 'slick', 'ngCookies', 'ngDialog', 'angular-centered', 'validation', 'validation.rule','chart.js','cgBusy','ngScrollbars', 'angular-drupal']);
...
(function() {
    ...
    instrument.$inject = ["$log", "$http", "Restangular", "backendCurrentUser"];
    angular
        .module('InnovationManagement')
        .factory('instrument', instrument);

    /** @ngInject */
    function instrument($log, $http, Restangular, backendCurrentUser) {
        var apiHost = 'http://localhost:3002/';
        var app = "giecaribe";
        var user = backendCurrentUser.getUser();
$log.debug("Log de user en Instrumet", user);
        ...
        function getAnswers(type) {
            //TODO: enviar token de usuario por header
            var new_answers = (JSON.parse(JSON.stringify(answers_structs[type])));
            if(user != null){
                $log.info("obteniendo instrumento" + type + " de :" + user.id);  
                return $http.get(apiHost + type + "?" + "user_id=" + user.id + "&app_id=" + app + "&auth_token=" + user.auth_token)
        ...
        function setAnswers(type, answers_new) {
            /*
            var data_to = {
                answers: {
            ...
            //verificar que el usuario del backend esta logeado:
            if (user.auth_token != null) {
                if (user.id != "" && answers_new != null) {
                    var index = 1;
                ...
                $log.debug("insertando el user_id:" + user.nit + " como: " + data_to + " en " + apiHost + type, data_to);
                ...
                //TODO: mostrar un modal solicitando usuario y contraseña
...
// .constant('user_nit', Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und[0].value) //logged drupal user
    ...
    backendCurrentUser.$inject = ["$http", "$log", "$cookies", "Restangular"];
    angular
        .module('InnovationManagement')
        .factory('backendCurrentUser', backendCurrentUser);

    /** @ngInject */
    function backendCurrentUser($http, $log, $cookies, Restangular) {
        var apiLogIn = 'http://localhost:3002/session/signin';
        var apiSingUp = 'http://localhost:3002/users/signup';
        var user = {};
        var token = null;
 ...
 //return $cookies.get('user_id');
            if (typeof(Drupal) !="undefined") { //Verificar que el modulo envio la info
                //verificar si hay informacion del perfil2 ""beneficiarios""
                if (Drupal.settings.instrumentos_gie.profile != null) {
                    $log.debug("drupal profiel", Drupal.settings.instrumentos_gie.profile);
                    var nit = "";
                    var id = "";
                    var empresa = "";
                    var backend_auth_token;
                    if (!angular.isUndefined(Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und)) {
                        nit = Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und[0].value ; 
                    }
                    if (!angular.isUndefined(Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nombre.und)) {
                        empresa = Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nombre.und[0].value ; 
                    } 
                    if (!angular.isUndefined(Drupal.settings.instrumentos_gie.user_api_data)) {
                        if(Drupal.settings.instrumentos_gie.user_api_data!=null){
                            backend_auth_token = Drupal.settings.instrumentos_gie.user_api_data.auth_token;
                            id = Drupal.settings.instrumentos_gie.user_api_data.id;  
                        }
                    } 

                    //tenemos dos token, el token del app y el token del usuario actual

                       
                      //  var email = Drupal.settings.instrumentos_gie.data.beneficiarios.field_nit.und[0].value;
                       // var imagen = Drupal.settings.instrumentos_gie.data.beneficiarios.field_imagen.und[0].uri;
                        //contruir usuario 
                    ...
                    //O no esta ingresando desde drupal o es un tipo de usuario distinto a beneficiario
                    return null;
                }
            }
            return null;
        ...
        function getName() {
            ...
            if (typeof(Drupal) != "undefined") { //Verificar que el modulo envio la info
                ...
                if (Drupal.settings.instrumentos_gie.profile != null) {
                    $log.debug("drupal profiel", Drupal.settings.instrumentos_gie.profile);
                    if (!angular.isUndefined(Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und)) {
                        var nit = Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und[0].value;
                        return nit;
                    }else{
                        //empresa no tiene nit
                        return false;
                    }
                    //TODO verificar que no sea nulo
                } else {
        ...
        if(typeof(Drupal)!='undefined'){ 
            if (Drupal.settings.instrumentos_gie.profile.beneficiarios.field_nit.und[0].value) {
                return true;
            } else {
                return false;
            }
    ...
    function auth($http, $log, $cookies, Restangular) {
       ...
       // var apiSingUp = 'https://giepiloto.herokuapp.com/users/signup';
        var apiLogIn = 'http://localhost:3002/signin';
        var apiSingUp = 'http://localhost:3002/signup';
        var user = {};
        var token = null;
...
(function() {
    ...
    InstrumentsController.$inject = ["$cookies", "$log", "drupal", "$scope"];
    angular
        .module('InnovationManagement')
         ...
         vm.saved = false;
                ...
                vm.save = function() {
                    if (!vm.saved) {
                        ...
                        if (instrument.setAnswers(vm.type, vm.answers)) {
...
(function() {
    ...
    IcaiController.$inject = ["auth", "instrument", "$scope", "$cookies", "$timeout", "$stateParams", "$log", "backendCurrentUser"];
    angular
        .module('InnovationManagement')
    ...
    /** @ngInject */
    function IcaiController(auth, instrument, $scope, $cookies, $timeout, $stateParams, $log, backendCurrentUser) {

        //inicializar variables independientes
        var vm = this;
        $scope.user = backendCurrentUser.getUser();
        $log.debug("En icai user: ", backendCurrentUser.getUser());
        //Suponer que el usuario tiene perfil "empresario"
        $scope.user_profile = true; 
        $scope.block_sensitive = false;
        ...
        vm.changePage = function(page) {
            //verificar si el usuario ya esta logeado: en este caso, si tiene nit en el backend
            if (backendCurrentUser.isLogged) {
                //obtener tab activ y el indice
                var active = vm.tabActive();
                var index = $.inArray(active, vm.pages);

                
                instrument.setAnswers("icai", $scope.answers);
        ...
        //Mensajes para pregunta answers.s3.p5 (ordenamiento)
        $scope.order = {};
        ...
        //otras funciones importantes, evaluar si se pueden globalizar
        var actualizar_nit = function(argument) {
            swal({ title: 'Hemos notado que nos falta tu NIT, por favor ingresalo', 
                html: '<p><input id="input-field" ng-model="answers.s1.p1">', 
                showCancelButton: true, 
                closeOnConfirm: false }, 
                function() { 
                   // backendCurrentUser.setNit($('#input-field').val());
                    $scope.$apply(function() { 
                     $scope.answers.s1.p1  =$('#input-field').val();
                    });               
                    swal({ html: 'Gracias! hemos actualizado tu información: <strong>' + $('#input-field').val() + '</strong>' 
                }); 
            });
        }
        ...
        if ($scope.user != null) { //SI es null, el usuario no esta logeado en el backend en ese caso abrir form e indicarlo
            $scope.user_data = Drupal.settings.instrumentos_gie.data; //debug
            $log.debug("GIE:", Drupal.settings.instrumentos_gie);
            console.log("Nit del usuario: " + $scope.user_nit);
            $scope.result = instrument.getAnswers("icai"); //no es necesario enviarle el id por que ya lo hace el servicio backend
            //Inicio del proceso de promesa de icai
            $scope.result.then(function(data) {
                $scope.user_id = $scope.user.id;
                //get response
                //$scope.user_id = $cookies.get("user_id");
                $scope.user_id = $scope.user_nit //por defecto se busca por nit, sino por id de consulta
                ...
                //$scope.answers = data;
                //verificar que data (la respuesta del promise) no sea nulo ni undefined
                if (!$.isEmptyObject(data) && data != null && typeof(data.s1) != "undefined") {
                    $scope.answers = data;
                    //Precargar el nombre de la empresa y el nit ademas bloquarlos para que no se puedan modificar.
                    $scope.block_sensitive = true;
                    $scope.answers.s11.p25 = $scope.user.empresa;
                    $scope.answers.s1.p1 = $scope.user.nit;
                    if($scope.user.nit == ""){ //Pedir el nit si por alguna razon no esta 
                        actualizar_nit();
                    }
                    ...
                    //props_watch();
                    //Se devolvio nulo de la base de datos o error
                }

                console.log("recibido en charact controller: " + $scope.answers);
            ...
            //sino cargar el instrumento vacio. indicar el estado del usuario como no logeado
...
(function() {
    ...
    ImiController.$inject = ["auth", "instrument", "$scope", "$cookies", "$timeout", "$stateParams", "backendCurrentUser"];
    angular
        .module('InnovationManagement')
        .controller('ImiController', ImiController);

    /** @ngInject */
    function ImiController(auth, instrument, $scope, $cookies, $timeout, $stateParams, backendCurrentUser) {
        var vm = this;
        //Configuraciones para el Grafico Mindex
        $scope.chart_options = {
            responsive: true,
        ...
        $scope.user = backendCurrentUser.getUser();//retorna un objeto usuario con id, token y mail y nit
        $scope.user_consult = $stateParams.id;
...
//Funciones comunes:
        ...
        $scope.swal_input = function(title, msg) {
             swal(title, msg);
        }
        $scope.refreshSlider = function() {
            $scope.$broadcast('refreshSlider');
        ...
        $scope.finish = function() {
            ...
            instrument.setAnswers("imi", $scope.answers);
        }
        ...
        var result = instrument.getAnswers("imi");
        if($scope.user != null){
...
(function() {
    ...
    AcapController.$inject = ["auth", "instrument", "$scope", "$cookies", "$timeout", "$stateParams", "$log", "backendCurrentUser"];
    angular
        .module('InnovationManagement')
        .controller('AcapController', AcapController);

    /** @ngInject */
    function AcapController(auth, instrument, $scope, $cookies, $timeout, $stateParams, $log, backendCurrentUser) {
        var vm = this;
        $scope.user = backendCurrentUser.getUser();
        $scope.finished = false;
        vm.panelTitle = "";
...
*/
        ...
        vm.changePage = function(page) {
            ...
            var active = vm.tabActive();
            var index = $.inArray(active, $scope.sessions);
            instrument.setAnswers("acap", $scope.answers);
     ...
     //   $scope.user_id = $cookies.get("user_id");
       ...
       var result = instrument.getAnswers("acap");
       if($scope.user != null){
           result.then(function(data) {
...
(function() {
    ...
    runBlock.$inject = ["$log", "$anchorScroll", "$rootScope", "$location", "$cookies", "auth", "$state", "backendCurrentUser", "$http"];
    angular
        .module('InnovationManagement')
        .run(runBlock);

    /** @ngInject */
    function runBlock($log, $anchorScroll, $rootScope, $location, $cookies, auth, $state,backendCurrentUser, $http) {
    

    //en cada ruta se configura el token de acceso al api, se supone que este token solo lo tiene el usuario
    //logeado en el backend, si envia un codigo errado, saldrá error
        var user = backendCurrentUser.getUser();
        if(user!=null){
                    $http.defaults.headers.common.Authorization = 'Token token=' +user.auth_token;
        }
        ...
        $rootScope.$on('$stateChangeStart', function(event, toState, toParams) {
            //TODO: si el api esta caido mostrar mensaje
/*
  ...
  function config($logProvider, $httpProvider, toastr,  $locationProvider,$validationProvider, RestangularProvider) {
   ...
   // RestangularProvider.setBaseUrl('https://surveyapi.herokuapp.com/');
    ...
    delete $httpProvider.defaults.headers.common["X-Requested-With"];
        //var user = backendCurrentUser.getUser();
     //$httpProvider.defaults.headers.common['Authorization'] = 'Token token=' +user.auth_token;
